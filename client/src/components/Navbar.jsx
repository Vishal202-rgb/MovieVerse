...
const Navbar = () => {
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'>
      ...
      >
        {/* Close icon (mobile) */}
        <FiX onClick={()=>{scrollTo(0,0),setIsOpen(false)}} className='md:hidden absolute top-6 right-6 w-6 h-6 cursor-pointer text-white' />
  ...
  !user ? (
    ...
    <UserButton>
      ...
      {/* Menu icon (mobile) */}
      <FiMenu onClick={()=>setIsOpen(!isOpen)} className='max-md:ml-4 md:hidden w-8 h-8 cursor-pointer' />
    </div>
  )
}
