...
//   if(!selectedTime){
//     return toast("Pahle time to select kro yrr")
//   }
...
//           <div key={item.time} onClick={()=>setSelectedTime(item)} className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition ${selectedTime?.time===item.time ? "bg-primary/80": "hover:bg-primary/10"}`}>
...
const SeatLayout = () => {
  ...
  const handleSeatClick = (seatId) => {
    if (!selectedTime) {
      return toast("Pehle time select karo yrr 😅")
    }
  ...
  return (
    <div className="flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-20 md:pt-32">
